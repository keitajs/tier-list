...
function Email(props) {
  ...
  const update = async () => {
    ...
    try {
      await axios.patch('http://localhost:2000/user/email', { email })
      props.setEdit(null)
      props.setUser(user => {
        user.email = email
        return user
      ...
      if (err?.response?.data?.message) return setErrors({ email: err.response.data.message })
      alert('Server error')
      console.log(err)
    }
  }
