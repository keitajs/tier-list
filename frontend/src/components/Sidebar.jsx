...
import { faBars, faHome, faUser, faList, faRightFromBracket, faPen } from '@fortawesome/free-solid-svg-icons'
...
function Sidebar(props) {
  ...
  return (
    <>
      ...
      <div className='fixed left-0 bottom-0 right-0 rounded-t-3xl shadow-lg overflow-hidden z-50 inline sm:hidden'>
        <div className='flex justify-between p-4 pb-3 bg-neutral-900'>
          <Link to={'/'} className='w-full flex flex-col items-center justify-center gap-0.5'>
            <FontAwesomeIcon icon={faHome} className='h-4' />
            <div className='text-xs'>Főoldal</div>
          </Link>
          <Link to={'/profile'} className='w-full flex flex-col items-center justify-center gap-0.5'>
            <FontAwesomeIcon icon={faUser} className='h-4' />
            <div className='text-xs'>Profil</div>
          </Link>
          {props.selectedList ? <>
            <Link to={'/list/editor'} className='w-full flex flex-col items-center justify-center gap-0.5'>
              <FontAwesomeIcon icon={faPen} className='h-4' />
              <div className='text-xs'>Editor</div>
            </Link>
          </> : <></>}
          <Link to={'/list'} className='w-full flex flex-col items-center justify-center gap-0.5'>
            <FontAwesomeIcon icon={faList} className='h-4' />
            <div className='text-xs'>Listák</div>
          </Link>
        </div>
      </div>
    </>
