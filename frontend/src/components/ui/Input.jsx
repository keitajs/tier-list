...
export default function Input({ children, label, name, type, value, onChange, message, reset, disabled, className }) {
  ...
  return (
    <div className="w-64 mt-1.5 lg:mt-6 flex flex-col">
      ...
      <div className='relative'>
        <input type={type === 'password' ? (showPass ? 'text' : 'password') : type} name={name} id={name} className={`w-64 px-3 pr-10 py-2 bg-neutral-700/75 rounded-lg outline-none ` + className} value={value} onChange={onChange} disabled={disabled} />
