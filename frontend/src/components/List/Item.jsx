  ...
  return (
    <div ref={setNodeRef} className='flex rounded-2xl overflow-hidden' style={style}>
      ...
      <div className={`relative select-text ${opened ? 'w-64 px-2' : 'w-0 px-0'} lg:h-32 md:h-28 h-24 py-1 bg-neutral-700 bg-opacity-60 rounded-e-2xl transition-all overflow-hidden`}>
        ...
        <p className='whitespace-nowrap text-lg leading-5 truncate opacity-50'>{props.anime.name}</p>
        <div className='flex gap-1.5 mt-1 ml-0.5'>
          <Link to={props.character.url} target='_blank' className='opacity-50 hover:opacity-75 transition-opacity'><FontAwesomeIcon icon={faEdit} /></Link>
          <Link to={props.anime.url} target='_blank' className='opacity-50 hover:opacity-75 transition-opacity'><FontAwesomeIcon icon={faEdit} /></Link>
          <Link className='opacity-50 hover:opacity-75 transition-opacity'><FontAwesomeIcon icon={faEdit} /></Link>
        </div>
      </div>
    </div>
  )
