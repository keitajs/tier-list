  ...
  return (
    <div className='flex items-center justify-between px-4 py-3 rounded-xl even:bg-neutral-950/70 odd:bg-neutral-950/85 hover:bg-neutral-950/40 transition-all'>
      ...
      <div className='flex gap-3'>
        <button className='group' onClick={() => props.setActive({ user: props.user, permission: props.permission })}><FontAwesomeIcon icon={faUserPen} className='text-emerald-600 group-hover:text-emerald-500 transition-colors' /></button>
        <button className='group'><FontAwesomeIcon icon={faUserMinus} className='text-rose-600 group-hover:text-rose-500 transition-colors' /></button>
      </div>
    </div>
  )
}
