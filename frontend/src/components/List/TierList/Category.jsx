...
function Category(props) {
  ...
  return (
    <div ref={props.permission?.move ? setNodeRef : null} className={`group/category flex my-1 rounded-2xl odd:bg-neutral-900/85 even:bg-neutral-900/100 ${props.permission?.move && isDragging ? 'cursor-grabbing opacity-25' : 'cursor-auto'}`} style={style}>
      ...
      <div className={`relative flex items-center justify-center lg:w-32 md:w-28 w-24 lg:min-w-32 md:min-w-28 min-w-24 lg:max-w-32 md:max-w-28 max-w-24 lg:min-h-32 md:min-h-28 min-h-24 text-center ${isLight(props.color) || edit ? 'text-white' : 'text-black'} bg-[--color]`} style={{'--color': props.color}}>
        {edit ?
