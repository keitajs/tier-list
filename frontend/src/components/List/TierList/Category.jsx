...
function Category(props) {
  ...
  return (
    <div ref={setNodeRef} className={`group/category flex my-1 rounded-2xl odd:bg-neutral-900/85 even:bg-neutral-900/100 ${isDragging ? 'cursor-grabbing opacity-25' : 'cursor-auto'}`} style={style}>
      ...
      <div className='relative flex items-center justify-center lg:w-32 md:w-28 w-24 lg:min-w-32 md:min-w-28 min-w-24 lg:max-w-32 md:max-w-28 max-w-24 lg:min-h-32 md:min-h-28 min-h-24 bg-[--color]' style={{'--color': props.color}}>
        {edit ? <>
          ...
          {props.name}
          <FontAwesomeIcon icon={faEdit} onClick={() => setEdit(true)} className='cursor-pointer absolute top-2 right-2 opacity-0 group-hover/category:opacity-40 hover:!opacity-100 transition-opacity' />
        </>}
