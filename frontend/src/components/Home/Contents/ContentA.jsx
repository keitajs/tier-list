...
function ContentA(props) {
  ...
  return (
    <div ref={ref} className={`absolute flex flex-col mx-6 ${props.left ? 'translate-x-[60%]' : props.right ? '-translate-x-[60%]' : ''} transition-all ${!props.center ? 'scale-50 opacity-0 pointer-events-none select-none' : ''}`}>
      <h2 className={`relative text-xl lg:text-3xl font-semibold w-max mb-5 pb-1 after:content-[""] after:absolute after:left-0 after:bottom-0 after:w-full after:h-0.5 after:rounded-lg after:bg-blue-500 ${props.center ? 'home-content-anim' : ''}`} style={{"--time": `${time}ms`}}>Lista készítés egyszerűen!</h2>
      ...
      <p className='my-1 leading-5 text-base lg:text-lg'>A <Link to={'https://myanimelist.net'} target='_blank' className='text-blue-200 hover:text-blue-100 transition-colors'>MyAnimeList</Link> oldalán található összes karaktert azonnal elérheted és felhasználhatod a listádban, csak a név szükséges a beazonosításához!</p>
