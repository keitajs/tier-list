...
function Lists({ history, logged, setSelectedList }) {
  ...
  const getUserLists = async () => {
    ...
    if (data.error || !data) return history('/')
  ...
  useEffect(() => {
    ...
    getUserLists().catch(err => console.error(err))
  }, [logged])
    ...
    <div className='flex xl:flex-row flex-col sm:min-h-[calc(100vh-3rem)] xl:h-[calc(100vh-3rem)] gap-6'>
      <div className='flex flex-col p-5 xl:w-1/2 w-full xl:h-full h-max rounded-3xl bg-neutral-900/85'>
        ...
        <div className='flex flex-col gap-2.5 grow -mr-2 pr-2 overflow-y-auto'>
          {lists !== null ? lists.map(list => <ListItem key={list.id} list={list} activeList={activeList} onClick={() => activeList?.id === list.id ? setActiveList(null) : setActiveList(list)} onDoubleClick={() => history(`/list/editor?id=${list.id}`)} />) : <></>}
      ...
      <div className='flex flex-col flex-1 gap-6 xl:w-1/2 w-full h-full max-h-full'>
        <div className='flex flex-col h-max p-5 rounded-3xl bg-neutral-900/85'>
          <ManageList history={history} setSelectedList={setSelectedList} setLists={setLists} activeList={activeList} setActiveList={setActiveList} />
        </div>
        <div className='flex flex-col flex-nowrap grow gap-5 min-h-0 p-5 rounded-3xl bg-neutral-900/85'>
          {activeList ? <ManagePermission setLists={setLists} activeList={activeList} setActiveList={setActiveList} /> : <OtherLists history={history} loaded={lists !== null} setSelectedList={setSelectedList} />}
        </div>
      </div>
    </div>
  )
